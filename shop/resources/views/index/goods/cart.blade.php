	...
	<div class="allgoods">
				...
				<div class="cart-body">
					<div class="cart-list">
						...
						<ul class="goods-list yui3-g">
							<li class="yui3-u-1-24"  goods_id="{{$v['goods_id']}}">
								<input type="checkbox" name="" id="" value=""class="box" />
							</li>
							<li class="yui3-u-11-24">
								<div class="good-item">
									<div class="item-img">
										@if($v['goods_img'])
											<img src="/storage/{{env('UPLOADS_URL')}}{{$v['goods_img']}}"/>
		...
		<div class="cart-tool">
			...
			<div class="toolbar">
				...
				<div class="sumbtn">
					<a class="sum-btn" href="{{url('goods/add')}}"class="jieshuan">结算</a>
				</div>
			</div>
		</div>
...
<script>
	$(document).on('click','.jieshuan',function(){
		...
		_box.each(function(index){
			goods_id+=$(this).parents('div').prev('div').
		})
	})
	function getMany(){
		var _box=$(".box:checkbox");
		var goods_id="";
		_box.each(function(index){
			goods_id+=$(this).parents("tr").attr('goods_id')+',';
		})
		goods_id=goods_id.substr("tr").attr(0,goods_id.length-1);
		$.ajax({
			url:"{{url('getmany')}}",
			type:"get",
